...
public class PRISMGenerator {
  private static String STRING_CHARS = "(?:\\\\[btnfru\"\'\\\\]|[^\\\\\"\'])";
  
  private static String STRING = (("([\"\'])(" + PRISMGenerator.STRING_CHARS) + "*)\\1");
  
  private HashMap<Object, Object> idToIntMap = CollectionLiterals.<Object, Object>newHashMap();
  
  private HashMap<String, Integer> nameToIntMap = CollectionLiterals.<String, Integer>newHashMap();
  
  private ArrayList<String> labelsList = CollectionLiterals.<String>newArrayList();
      ...
      for(final Integer i : _upTo) {
        {
          ...
          if (!_notEquals) {
            ...
            State _get_1 = _states_2.get((i).intValue());
          ...
          if (_and) {
            ...
            State _get_2 = _states_3.get((i).intValue());
            String _name_2 = _get_2.getName();
            Integer _put = this.nameToIntMap.put(_name_2, i);
            _builder.append(_put, "");
            _builder.newLineIfNotEmpty();
          }
        }
  ...
  public String replace(final String s) {
    String result = s;
    Set<Map.Entry<String, Integer>> _entrySet = this.nameToIntMap.entrySet();
    for (final Map.Entry<String, Integer> entry : _entrySet) {
      {
        String _key = entry.getKey();
        String _replace = _key.replace("\"", "\\\\\"");
        String name = _replace.replace("\'", "\\\\\'");
        Integer _value = entry.getValue();
        String _plus = ("s=" + _value);
        String _replaceAll = result.replaceAll((("State:([\"\'])" + name) + "\\1"), _plus);
        result = _replaceAll;
      }
    }
    for (final String entry_1 : this.labelsList) {
      String _replace = entry_1.replace("\"", "\\\\\"");
      String _replace_1 = _replace.replace("\'", "\\\\\'");
      String _plus = ("Label:([\"\'])" + _replace_1);
      String _plus_1 = (_plus + "\\1");
      String _replaceAll = entry_1.replaceAll("[^a-zA-Z0-9_]", "_");
      String _plus_2 = ("\"" + _replaceAll);
      String _plus_3 = (_plus_2 + "\"");
      String _replaceAll_1 = result.replaceAll(_plus_1, _plus_3);
      result = _replaceAll_1;
    }
    String _replaceAll_2 = result.replaceAll(("State:" + PRISMGenerator.STRING), "false");
    result = _replaceAll_2;
    String _replaceAll_3 = result.replaceAll(("Label:" + PRISMGenerator.STRING), "false");
